...
class TrainingArguments(transformers.TrainingArguments):
    cache_dir: Optional[str] = field(default='/home/jhjeong/Anomaly-OneVision/pretrained_models/')
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        # if the largest dimension is larger than 1024, resize it to 1024 while keeping the aspect ratio
        if max(image.size) > 1024:
            if image.size[0] > image.size[1]:
                new_size = (1024, int(1024 * image.size[1] / image.size[0]))
            else:
                new_size = (int(1024 * image.size[0] / image.size[1]), 1024)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        # Only support 7B and 0.5B models for now
        if '7b' in model_args.model_name_or_path:
            rank0_print("Using 7B model!!!")
            expert_path = './pretrained_expert_7b.pth'
        else:
            rank0_print("Using 0.5B model!!!")
            expert_path = './pretrained_expert_05b.pth'

        rank0_print(f"Loading anomaly encoder from pretrained weights {expert_path}...")
        ...
        anomaly_encoder.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_mlp_adapter" in tunable_parts:
                for p in model.get_model().mm_projector.parameters():
                    p.requires_grad = True
            if "anomaly_encoder" in tunable_parts:
                for p in model.get_model().anomaly_encoder.parameters():
                    p.requires_grad = True
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "anomaly_encoder" not in name:
                        param.requires_grad_(True)
            #"""
            for name, param in model.named_parameters():
                rank0_print(f"Inspecting {name} and {param.requires_grad}")

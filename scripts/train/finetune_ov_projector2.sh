...
BASE_RUN_NAME="anomaly_ov3_7B_only_llm_and_projector_all_data2"
...
RUN_NAME="anomaly_ov3_7B_only_llm_and_projector_all_data2" 
...
ACCELERATE_CPU_AFFINITY=1 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --data_path /data/02/jiacong/data/our_datasets2_new.yaml \
    ...
    --bf16 True \
    --run_name $RUN_NAME \
    --output_dir /data/02/jiacong/anomaly_detection/Anomaly_OV3_7b/all_data_checkpoints2/$RUN_NAME \
    --num_train_epochs 1 \

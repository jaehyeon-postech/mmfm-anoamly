...
BASE_RUN_NAME="anomaly_ov3_7B_only_llm_and_projector_all_data"
...
RUN_NAME="anomaly_ov3_7B_only_llm_and_projector_all_data" 
...
ACCELERATE_CPU_AFFINITY=1 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --bf16 True \
    --run_name $RUN_NAME \
    --output_dir /data/02/jiacong/anomaly_detection/Anomaly_OV3_7b/checkpoints/$RUN_NAME \
    --num_train_epochs 1 \
